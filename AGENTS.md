...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr=, the forge's pr_head= when available, and an optional gh_account= naming the GitHub login whose stored gh token the merge watch and merge must use (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak .watch-last-close   watcher internals; never touch
...
Load the captain-invocable `design-directions` skill before scoping or dispatching a request to explore, compare, or choose among multiple visual design directions for a web project.
Load `design-review` before critiquing a design, when the captain reports that something looks wrong or unfinished, and before a crewmate's design work lands.
...
Once merge authority is granted, run the landing chain with `bin/fm-land.sh <task-id>`: it merges the recorded PR, refreshes the clone, tears the task down, completes the backlog item, and prints the ready cue, stopping at the first failure with evidence and resuming from the first incomplete step on re-run.
...
- `design-review` - load before critiquing a web design's visual quality, when the captain reports that something looks wrong or unfinished, and before a crewmate's design work lands.
