...
`bin/fm-effort-lib.sh` is the single executable owner of the accepted per-harness effort levels that spawn and dispatch validation both consume; when a row's effort levels change, update that lib in the same change.

| Harness | Model flag | Effort flag | Notes |
|---|---|---|---|
| claude | `--model <model>` | `--effort <low\|medium\|high\|xhigh\|max>` | Verified on Claude Code 2.1.196. |
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified on codex-cli 0.146.0. The installed binary schema contains `model_reasoning_effort` and the active config uses it. `max` was verified with a live `codex exec -m gpt-5.6-luna -c 'model_reasoning_effort="max"'` probe that returned a correct answer with no invalid_request_error, superseding 0.142.1's catalog-only evidence that had omitted it. |
