...
A busy pane is otherwise exempt from staleness, but only until one of two progress bounds is crossed: its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS` (its `state/<id>.meta` spawn record before any turn completes) - the overlong-turn alarm - or, with no turn-ended file ever touched, the busy source is still the `fm-spawn` launch seed past `FM_NEVER_STARTED_SECS` - the never-started alarm.
Both route through the same wedge escalation with a distinguishing reason ("no completed turn" and "never started"), the escalation count, and the `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
A declared pause or captain-held wait exempts both; an actively-running no-mistakes run-step (a pipeline-owned wait) exempts the overlong-turn bound.
