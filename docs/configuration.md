...
FM_BUSY_TURN_MAX_SECS=1200         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the overlong-turn wedge escalation (distinguishing reason "no completed turn") takes over; exempt while the task drives an actively-running no-mistakes run-step or declares a pause/captain-held wait; inspection-only, never an automatic interrupt or restart
FM_NEVER_STARTED_SECS=120          # a busy pane whose semantic source is still the fm-spawn launch seed, with no state/<id>.turn-ended file ever touched, escalates as never-started (distinguishing reason "never started") past this spawn age; short by design because a never-started worker produces nothing
